...
export function findTheBat(contacts: Contact[]): string | null {
    const batmanContact = contacts.find(contact => contact.name === 'Batman');

    return batmanContact ? batmanContact.address : null;
}
 ...
 */
export function getUser(id: number, users: { [key: number]: User }): User | null {
